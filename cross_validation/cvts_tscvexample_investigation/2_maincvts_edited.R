# https://raw.githubusercontent.com/zachmayer/cv.ts/master/R/cv.ts.R
...
cv.ts <- function(x, FUN, tsControl=tseriesControl(), xreg=NULL, progress=TRUE, packages=NULL, ...) {
  ...
  forecasts <- foreach(i=steps, .combine=combine, .multicombine=FALSE,
                       .packages=c('forecast', 'caret'), .export=c('testObject', 'tsSummary', 'tseriesControl')) %dopar% {
                           ...
                           out <- auto.arimaForecast(xshort, h=12, ic='aicc', stepwise=FALSE)
